...
class Petrol(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.image = pygame.transform.smoothscale(self.image,(60,55))
        self.rect = self.image.get_rect()
        self.rect.midbottom = (random.randint(220,WIDTH -220),-1100)
        self.mask = pygame.mask.from_surface(self.image)
...
while True:
    ...
    petrol_hit = pygame.sprite.spritecollide(P1, petrols, False, pygame.sprite.collide_mask)
    if petrol_hit:
        for petrol in petrol_hit:
            SCORE += 1                     
            collect.play()  
            petrol.rect.top = 0      
            petrol.rect.midbottom = (random.randint(220,WIDTH-220), -1100)
